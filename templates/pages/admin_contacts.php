...
<section class="section" style="padding-top: 4rem; padding-bottom: 4rem;">
    <div class="container" style="max-width: 1280px;">
        <div style="display: flex; gap: 1rem; margin-bottom: 2.5rem; justify-content: center; background: rgba(255, 255, 255, 0.5); padding: 0.5rem; border-radius: 50px; border: 1px solid rgba(0, 0, 0, 0.05); width: fit-content; margin-left: auto; margin-right: auto;">
            <a href="/admin/blog" class="admin-nav-tab" style="padding: 0.8rem 2rem; text-decoration: none; color: var(--primary-color); border-radius: 40px; transition: all 0.3s; font-weight: 600; display: flex; align-items: center; gap: 0.8rem;">
                <i class="fas fa-edit" style="font-size: 1.1rem;"></i> Blog
            </a>
        ...
        <div class="glass-card" style="margin-bottom: 2rem;">
            <button id="toggleAddForm" style="width: 100%; padding: 1rem; background: linear-gradient(135deg, rgba(139, 92, 246, 0.2), rgba(236, 72, 153, 0.2)); border: 1px solid var(--primary-color); border-radius: 10px; color: var(--primary-color); cursor: pointer; font-size: 1rem; font-weight: 600; display: flex; align-items: center; justify-content: space-between;">
                <span><i class="fas fa-plus-circle"></i> Add New Contact</span>
                <i class="fas fa-chevron-down" id="toggleIcon" style="transition: transform 0.3s;"></i>
            </button>

            <form id="addForm" action="/admin/contacts/add" method="post" style="display:none; padding-top: 1.5rem; gap: 1rem;">
...
<script>
    ...
    function applyTabStyles(activeId, inactiveId) {
        ...
        active.style.boxShadow = '0 4px 15px rgba(47, 79, 79, 0.3)';

        inactive.style.background = 'transparent';
        inactive.style.color = 'var(--primary-color)';
    ...
    function switchTab(tabName) {
        ...
        const assessments = document.getElementById('assessments-content');

        if (tabName === 'contacts') {
            contacts.style.display = 'block';
            assessments.style.display = 'none';
            applyTabStyles('tab-contacts', 'tab-assessments');
        } else {
            contacts.style.display = 'none';
            assessments.style.display = 'block';
            applyTabStyles('tab-assessments', 'tab-contacts');
        }
    }
    ...
    window.addEventListener('click', function (event) {
        ['editContactModal', 'deleteContactModal', 'editAssessmentModal', 'deleteAssessmentModal'].forEach(function (id) {
